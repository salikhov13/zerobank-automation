...
//Converts webelements list to dates list
public static List<Date> webElementListToDateList(List<WebElement> webElementsList, String delimeter)
{
    AccountActivityPage accountActivityPage = new AccountActivityPage();
    List<String> stringList = BrowserUtils.getElementsText(webElementsList);
    ...
    for (String s : stringList) {
        ...
        for (int i = 0; i < sArray.length; i++) {
            String s1 = sArray[i];
            iArray[i] = Integer.parseInt(s1);
        }
        Calendar myCalendar = new GregorianCalendar(iArray[0], iArray[1] -1, iArray[2]);
        dateList.add(myCalendar.getTime());
    }
    return dateList;
}
public static Date stringToDate(String str, String delimeter){
        String[] sArray = str.split(delimeter);
        int[] iArray = new int[sArray.length];
    for (int i = 0; i < sArray.length; i++) {
